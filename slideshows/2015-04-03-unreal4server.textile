 ...
 * GameMode:定义游戏规则，只存在于服务器中
 * GameState:当前游戏的状态，客户端可以获得的游戏信息。
 * PlayerState:游戏参与者的信息，例如玩家或者AI。
 ...
 * 位置同步
  ** 角色移动组件("Character Movement Component":https://docs.unrealengine.com/latest/INT/Gameplay/Networking/CharacterMovementComponent/index.html)
   *** 移动的处理过程：
    **** 客户端每个Tick，都会调用TickComponent()，计算出当前帧的加速度和旋转，然后调用 ReplicateMoveToServer()。
	**** ReplicateMoveToServer() 将所有移动操作保存在一个PendingMove list中，然后调用PerformMovement()，并且将这些移动操作通过调用ServerMove()同步给服务器（传递移动参数，客户端最终位置和时间戳）。
	**** ServerMove()实在服务端执行的，该函数解析出移动参数，模拟相同的移动，然后检查结果和客户端发送的结果是否相同，并且和上一次客户端反馈的时间是否已经相隔了很久，如果位置相差足够明显，服务器将会调用ClientAdjustPosition()。
	**** ClientAdjustPosition()在客户端执行，将客户端的位置同步到服务器给定的位置（相当于手机魔域的踢回），设置bUpdatePosition标记为true。
	**** 当TickComponent() 在客户端再次调用时，如果bUpdatePosition的值为true,客户端将会先调用ClientUpdatePosition()，之后再调用PerformMovement()（这里可以理解重复整个流程）。
	**** ClientUpdatePosition()将会重放 pending move list中服务器调整的时间戳之后的所有移动。
   *** 新增一个传送技能的示例
